...
class RangerModel {
  ...
  _stMergeAttrs(query) {
    ...
    if (query.region) {
      ...
      let bedString = this._buildBedFile(query.multiregions_key);
      // let options = config.provide();
      // let tmpDir = options.get('tempdir');
      let tmpDir = config.provide().get('tempdir');
      fse.writeFileSync(tmpDir + '/testbed.bed', bedString);
      attrs.push('-L');
      attrs.push(tmpDir + '/testbed.bed'); // Change location and name later
    }
