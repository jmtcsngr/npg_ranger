...
class RangerModel {
  ...
  _stMergeAttrs(query) {
    ...
    if (query.region) {
      ...
      // Would be query.referenceName, query.start, and query.end normally
      query.multiregions_key.forEach( region => { // Check high level spec tests for the only existing edited test
        bedString += region.referenceName + "\t" + region.start + "\t" + region.end + "\n";
      });
      fse.writeFileSync('/tmp/testbed.bed', bedString);
      // Finished building bed file
      attrs.push('-L');
      attrs.push('/tmp/testbed.bed'); // Change location and name later
      attrs.push('--reference', RangerModel.fixReference(query));
    }
    attrs.push('-');
